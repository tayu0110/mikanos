...
namespace usb::xhci {
    ...
    Error EventRing::Initialize(size_t buf_size, InterruptRegisterSet *interrupter) {
        if(buf_ != nullptr) FreeMem(buf_);
        ...
        ERSTSZ_Bitmap erstsz = interupter_->ERSTSZ.Read();
        erstsz.SetSize(1);
    ...
    void EventRing::Pop() {
        ...
        TRB *segment_end
            = segment_begin + erst_[0].bits.ring_setment_size;
