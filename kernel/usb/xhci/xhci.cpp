...
namespace {
    ...
    void InitializeEP0Context(EndpointContext &ctx, Ring *transfer_ring,
        ...
        ctx.bits.SetTransferRingBuffer(transfer_ring->Buffer());
    ...
    Error AddressDevice(Controller &xhc, uint8_t port_id, uint8_t slot_id) {
        ...
        const auto ep0_dci = DeviceContext(0, false);
    ...
    Error OnEvent(Contoller &xhc, CommandCompletionEventTRB &trb) {
        ...
        Log(kDebug, "CommandCompletionEvent: slot_id = %d, issuer = %s\n",
                trb.bits.slot_id, kTRBTypeName[issuer_type]);
...
namespace usb::xhci {
    ...
    Error Controller::Initialize() {
        ...
        iman.bits.interrupter_pending = true;
        iman.bits.interrupter_enable = true;
