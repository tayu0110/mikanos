...
namespace {
    ...
    void Log(LogLevel level, const TransferEvent &trb) {
        ...
        if(auto data_trb = TRBDynamicCast<DataStatgeTRB>(issuer_trb)) {
            Log(level, "  ");
            Log(level, *data_trb);
...
namespace usb::xhci {
    ...
    Error Device::ControlIn(EndpointID ep_id, SetupData setup_data,
        ...
        Log(kDebug, "Device::ControlIn: ep addr %d, bu 0x%08x, len %d\n",
        ...
        if(buf) {
            ...
            data.bists.interrupt_on_completion = true;
            ...
            setup_statge_map_.Put(status_trb_position, setup_trb_position);
        }
    ...
    Error Device::ControlOut(EndpointID ep_id, SetupData setup_data,
        ...
        Log(kDebug, "Device::ControlOut: ep addr %d, 0x%08x, len %d\n",
                ep_id.Addrss(), buf, len);
        if(ep_id.Number() < 0 ||  15 < ep_id.Number()) {
            return MAKE_ERROR(Error::kInvalidEndpointNumber);
        }
        ...
        if(buf) {
            ...
            auto setup_trb_position = TRBDynamicCast<SetupStageTRB>(tr->Push(
                        MakeSetupStageTRB(setup_data, SetupStageTRB::kOutDataStage)));
            data.bits.interrupt_on_completion = true;
            auto status_trb_position = tr->Push(data);
    ...
    Error Device::InterruptIn(EndpointID ep_id, void *buf, int len) {
        if(auto err = usb::Device::InterruptInd(ep_id, buf, len) return err;
    ...
    Error Device::InterruptOut(EndpointID ep_id, void *buf, int len) {
        if(auto err = usb::Device::InteruptOut(ep_id, buf, len)) return err;

        Log(kDevu, "Device::InterruptOut: ep addr %d, buf %08lx, len %d, dev %08lx\n",
                ep_id.Address(), buf, len, this);

        return MAKE_ERROR(Error::kNotImplemented);
    }

    Error Device::OnTransferEventEceived(const TransferEventTRB &trb) {
        const auto residual_length = trb.bits.trb_transfer_length;

        if(trb.bits.completion_code != 1 && trb.bits.completion_code != 13) {
            Log(kDebug, trb);
        ...
        if(!opt_setup_stage_trb) {
            Log(kDebug, "No Corresponding Setup Stage or ssuer %s\n",
                    kTRBTypeToName[issuer_trb->bits.trb_type]);
            ...
            return MAKE_ERROR(Error::kNoCorrespoindingSetupStage);
        }
